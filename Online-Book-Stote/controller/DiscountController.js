   ...
   async getALL(req,res){
    try {
      ...
      if (!discount) {
        ...
        return res.status(HTTP_STATUS.NOT_FOUND).send(failure("There is no discount to show."));
     }
     return res.status(HTTP_STATUS.OK).send(success("Successfully received all discount's data",{discounts:discount,total:discount.length}));
    } catch (error) {
      console.error(error);
    ...
    async update(req,res){
        try {
            ...
            if (!discount) {
              ...
              return res.status(HTTP_STATUS.NOT_FOUND).send(failure("There is no discount object was fount to update."));
           }

            const { bookID, discountPercentage, startDate, endDate} = req.body;
            const book= await DiscountModel.findOne({bookID: bookID });
            if (!book) {
