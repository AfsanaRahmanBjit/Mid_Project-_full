...
class CartController{
    async getCart(req, res) {
      try {
          ...
          return res.status(HTTP_STATUS.OK).send(success("Successfully got cart for user",{cart:cart}));
      } catch (error) {
          console.log(error);
    ...
    async removeBookFromCart(req, res) {
      try {
          ...
          return res.status(HTTP_STATUS.OK).send(success(`Book with ID ${bookID} deleted from cart successfully`, { cart: cart }));
      } catch (error) {
          console.error(error);
          return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).send(failure("Internal Server Error"));
      }
